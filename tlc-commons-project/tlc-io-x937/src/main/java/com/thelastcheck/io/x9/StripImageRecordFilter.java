...
public class StripImageRecordFilter implements RecordFilter {
    ...
    public StripImageRecordFilter() {
    }

    public Record filter(Record record) {
        if (record instanceof X937ImageViewDataRecord) {
            X937ImageViewDataRecord ivdr = (X937ImageViewDataRecord) record;
            int length = 0;
            try {
                length = ivdr.lengthOfImageDataAsInt();
            } catch (InvalidDataException e) {
            }
            if (length > 0) {
                ivdr.imageData(new ByteArray(0));
            }
        }
        return record;
    }
